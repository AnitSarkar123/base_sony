          ...
          />
              ...
              <div className="flex-shrink-0 aspect-square">
                <Button
                  ...
                  className="h-[60px] w-[60px] flex-col gap-0 p-1 bg-gray-100/70 rounded-sm border border-white/20"
                  onClick={() => {
                    // Just increment resetKey and reset pagination
                    // The useEffect will handle refetching filters
                    ...
                    // Refetch cars with reset filters
                    setIsLoading(true);
                    
                    // Fetch cars with reset filters
                    axios.get(
                      `/api/home-japan?page=1&limit=${pagination.limit}`,
                      {
                        headers: {
                          "Cache-Control": "no-cache",
                          cache: "no-cache",
          ...
          <div className="relative">
            {isLoadingMore && (
              <div className="absolute inset-0 flex items-center justify-center">
                <Loader className="h-8 w-8 text-black" />
              </div>
            )}
            <button 
              onClick={async () => {
                ...
                try {
                  ...
                  const featuresParam = pagination.selectedFeatures && pagination.selectedFeatures.length > 0 
                    ? `&features=${pagination.selectedFeatures.map(f => f.name).join(',')}` 
                    : '';
                  const sortParam = pagination.sortBy 
                    ? `&sortBy=${pagination.sortBy.name}:${pagination.sortBy.order}` 
                    : '';
                  
                  const { data } = await axios.get(
                    `/api/home-japan?page=${nextPage}&limit=${pagination.limit}${detailsQueryParam}${featuresParam}${sortParam}`,
                    {
                      headers: {
                        "Cache-Control": "no-cache",
                        cache: "no-cache",
		...
		<InventoryFilterPopup 
		  ...
		  onApply={(values) => {
			if (activeInventoryFilter) {
			  handleInventoryFilterChange(activeInventoryFilter, values);
