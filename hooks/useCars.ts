...
export const useCars = () => {
  ...
  // Check if we're viewing the Japan section on the home page
  const isViewingJapanSection = typeof window !== 'undefined' && 
    sessionStorage.getItem('viewingJapanSection') === 'true';
  ...
  >(["cars", pathname, isViewingJapanSection], fetchCars, {
    ...
    onSuccess: async (response) => {
      ...
      // Don't scroll to top if we're in the Japan section of the home page
      if (!isViewingJapanSection) {
        window.scrollTo(0, 0);
      }
    },
    enabled:
      pathname.includes("/inventory") ||
      pathname.includes("/japan") ||
      isViewingJapanSection ||
